...
//define("DEBUG",TRUE);
echo "<html><head><title>Quan li bai tap</title></head><body>";
if (isset($_GET['act'])) {
	$act = $_GET['act'];
	if ($act=="add") {
...
function themdebai($tieude,$muc,$noidung) {
	$sql= "insert into baitap (ten,muc,noidung)value ('$tieude', '$muc','$noidung')";
...
function capnhatdebai($bai,$tieude,$muc,$noidung) {
	$sql= "update baitap set ten='$tieude', muc='$muc',noidung='$noidung' where id=$bai";
	if (defined("DEBUG")) echo $sql."<br/>";
	$result = mysql_query($sql) or die (mysql_error());
	if ($result) echo "THÀNH CÔNG";
	else echo "THẤT BẠI";
...
function suadebai($bai) { //xem de bai
	...
	if (mysql_num_rows($result)) {
		...
		echo "Tiêu đề <input name='tieude' type='text' size='100' value='". $data['ten']."'><br/>";
		echo "Mục <input name='muc' type='text'  value='". $data['muc']."'><br/>";
		echo "Nội dung <textarea name='noidung' rows='9' cols='70'>".$data['noidung']."</textarea><br/>";
...
function xoadebai($bai) {
	$sql= "delete from baitap where id=$bai";
	if (defined("DEBUG")) echo $sql."<br/>";
	$result = mysql_query($sql) or die (mysql_error());
	if ($result) echo "THÀNH CÔNG";
	else echo "THẤT BẠI";
...
function lietkedebai($trang=1,$sl1trang=20){ //liet ke de bai theo trang
	...
	$sql="select * from baitap limit ".($trang-1)*$sl1trang.",$sl1trang";
	if (defined("DEBUG")) echo $sql."<br/>";
	$result = mysql_query($sql) or die(mysql_error()); 
	if (mysql_num_rows($result)) {
		
		while ($data = mysql_fetch_array($result)){
			echo "<a href='?act=view&id=".$data['id']."'>".$data['id'].". ".$data['ten']."</a> //<a href='?act=view&muc=".$data['muc']."'>".$data['muc']."</a><br/>\n";
			//echo "OK";
		}
	} else {
